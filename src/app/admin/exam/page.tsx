...
const ExamPage = () => {
  ...
  const [editingExam, setEditingExam] = useState<AdminExamData | null>(null);
  const [formData, setFormData] = useState<Partial<AdminExamData>>({
    title: "",
    subject: "",
    difficulty: 1,
  });
  ...
  useEffect(() => {
    const fetchExams = async () => {
      try {
        ...
        setExams(response.items.$values || []);
  ...
  // Filter exams based on search term (client-side for now)
  ...
  // Use filtered exams for display
  ...
  const openModal = (exam?: AdminExamData) => {
    if (exam) {
      setEditingExam(exam);
      setFormData({
        title: exam.title,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.subject || formData.difficulty === undefined) {
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50">
                  ...
                  <TableCell className="font-semibold text-gray-900 px-6 py-4">Created By</TableCell>
                  ...
                  <TableRow key={exam.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <TableCell className="px-6 py-4 text-sm text-gray-900 font-medium">
                      <div>
                        ...
                        <div className="text-gray-500 text-xs mt-1">{exam.status}</div>
                      </div>
                    </TableCell>
                    <TableCell className="px-6 py-4 text-sm text-gray-600">{exam.subject}</TableCell>
                    <TableCell className="px-6 py-4 text-sm text-gray-900 text-center">{exam.questionCount}</TableCell>
                    <TableCell className="px-6 py-4 text-sm text-gray-600">{exam.createdBy}</TableCell>
                    <TableCell className="px-6 py-4 text-sm">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        exam.difficulty === 1 ? 'bg-green-100 text-green-800' :
                        exam.difficulty === 2 ? 'bg-yellow-100 text-yellow-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {exam.difficulty === 1 ? 'Easy' : exam.difficulty === 2 ? 'Medium' : 'Hard'}
                      </span>
                    </TableCell>
